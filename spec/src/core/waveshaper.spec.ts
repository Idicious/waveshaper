import { WaveShaper, AudioInterval, defaultConfig } from "../../../src";
...
describe("WaveShaper class tests", () => {
  ...
  it("Ignores invalid options.", () => {
    ...
    expect(waveshaper.options.samplesPerPixel).toBe(
      defaultConfig.samplesPerPixel
    );
  ...
  it("Can add AudioData.", done => {
    ...
    fetch("assets/audio/test.wav")
      ...
      .then(data => {
        manager
          .setData({ id: "1", data: data.getChannelData(0) })
          .process()
  ...
  it("Stores the last process result.", () => {
    ...
    manager.setTracks(...tracks).process();
  ...
  it("Ignores flatten on non existant tracks", () => {
    ...
    const manager = new WaveShaper();
    manager.setTracks(...tracks).flatten(["3"]);
  });
});
